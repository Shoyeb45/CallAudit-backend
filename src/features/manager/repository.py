...
class ManagerRepository:
    ...
    def activate_auditor(self, auditor_id: str) -> bool:
        try:
            ...
            logger.debug(f"Activating auditor: {auditor.id} (current status: {auditor.is_active})")
    ...
    def activate_counsellor(self, counsellor_id: str) -> bool:
        try:
            ...
            logger.debug(f"Activating counsellor: {counsellor.id} (current status: {counsellor.is_active})")
            counsellor.updated_at = datetime.utcnow()
            ...
            logger.error(f"Failed to activate counsellor from database, error: {str(e)}")
            return False

    def unflag_audit(self, audit_id) -> bool:
        try:
            report = self.db.query(AuditReport).filter(AuditReport.id == audit_id).first()
